...
description: Pick the right library for a given frontend task from a curated, opinionated list, numbers, OTP inputs, charts, command menus, virtualization, drag and drop, toasts, state, styling, and more. Use when a task needs a component rather than an animation (a toast, a drawer, a command menu, a dropdown) and the choice is between hand-rolling it and installing a library; animate hands component choices here.
---
