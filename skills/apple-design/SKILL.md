---
name: apple-design
description: The reference for Apple's interface reasoning and the physics behind fluid motion, translated for the web: velocity handoff, momentum projection, rubber-banding, damping, translucent materials and depth, typography (optical sizing, tracking, leading), reduced motion, and the foundations (feedback, spatial consistency, restraint). Read it when the brief asks for an Apple-style or Apple-adjacent feel, or when a gesture-driven interaction needs its reasoning and numbers; the implementation is then written by animate on the web or animate-expo on React Native.
---
