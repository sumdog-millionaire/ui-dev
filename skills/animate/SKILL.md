---
name: animate
description: Build a web animation from scratch, making the decisions in the order that determines whether it feels right, should it animate at all, what purpose, which tool, which properties, which curve and duration, how it interrupts, how it exits. Writes the implementation and owns every motion constant on the web, so any skill that needs interface motion hands the task here. Use when asked to animate something, add motion, make a component feel alive, or build a transition on a web page or web app. For React Native or Expo use animate-expo; for critiquing existing motion use review-animations; for auditing a whole codebase use improve-animations; for Apple's physics and reasoning read apple-design.
---
