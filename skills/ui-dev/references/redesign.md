...
Read when the surface already exists and is being changed. Misclassifying the kind of change is the
biggest source of bad redesign output, so the first action is to name it.
...
If ambiguous, ask once: "Should this redesign preserve the existing brand, or are we starting
...
- **SEO baseline**: ranking pages, meta titles, structured data, social cards. SEO migration is the first redesign risk.
...
Sound architecture, content and SEO call for targeted evolution (levers 1 to 4: most of the value
at a fraction of the risk). Structural visual debt (broken architecture, no design system, broken
mobile) calls for a full redesign with strict content preservation. A changing brand is greenfield.
