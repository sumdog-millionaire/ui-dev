...
here. The test is whether a category-fluent user trusts the interface at once or pauses at every
subtly-off component. The failure mode is not flatness but strangeness without purpose:
over-decorated buttons, mismatched controls, gratuitous motion, display fonts where labels should
be, invented affordances for standard tasks. The tool disappears into the task.

## The five lenses

Posed on every screen, in this order, before anything else:
...
5. **Be opinionated.** The point of view is what sells; lead with the claims only this product can make.

## Space

- A pane a person can resize fills its space and follows the divider. A document opened to be read inside the app is capped with its holder at 65 to 75 characters. Measure the element, never the window.
...
- Colour defaults to restrained. Accent for primary actions, current selection and state indicators; a second neutral layer for sidebars, toolbars and panels; a standardised semantic vocabulary for hover, focus, active, disabled, selected, loading, error, warning, success and info. Inactive states never carry heavy or full-saturation colour.
- System fonts and familiar sans defaults are permitted without apology.
...
- Helper text answers an implicit question rather than restating the control.

## States

- Every control has default, hover, focus, active, disabled, loading, error and success behaviour; visible keyboard focus, logical tab order, labels, and platform-sized touch targets.
...
- Match neighbouring mental models: terminology, disclosure, routing, save behaviour, optimistic or pessimistic patterns, all the same screen to screen. If the save button looks different in two places, one is wrong.

## Motion and delight

Motion conveys state, feedback, loading and reveal, nothing else; routine transitions stay fast and
there is no page-load choreography. A control used a hundred times a day gets none. Delight
concentrates at the meaningful moments, first use, completion, recovery, mastery, and matches the
effort and consequence; reliability carries everything else. Never fake work or delay completion to
stage a flourish. `animate` writes whatever moves.

## What operate can afford that persuade cannot
