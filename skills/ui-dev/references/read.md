...
- One reading path per article; related links gather into one block at the end rather than scattering mid-flow.
- Documentation sidebars show at most four sibling choices per level before grouping.
- Headings are the outline: obvious steps of scale and weight, a heading with more space above than below, and never two levels that look alike.
- Colour encodes wayfinding and reading hierarchy; rarity gives the accent force. Links look like links.
- Code blocks and tables may be wider than the column, each inside its own horizontal scroller; the page never scrolls sideways.
