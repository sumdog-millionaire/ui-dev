...
the working state. Then load `grilling` and `domain-modeling` together, never one without the other (the
pair `/grill-with-docs` runs), always, with the brief, the findings, the brand constraints and the open
