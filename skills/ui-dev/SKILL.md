---
name: ui-dev
description: Design and build distinctive websites and web applications with Taste, optional local brand packs, Stitch exploration and browser verification. Use for new interfaces, redesigns, frontend changes, brand onboarding and reviewing upstream Taste upgrades; adapt visual expression to the task, including functional dashboards.
---

# ui-dev: Taste-based web design and implementation

> All web surfaces, from expressive websites to functional applications. Use the same design reasoning; vary the amount of stylistic expression.
> Every aesthetic rule below is a default, not a substitute for the brief or an approved brand. Accessibility, security and factual accuracy remain requirements.

## Start here: workflow and authority

For an upstream Taste upgrade request, read [upgrades.md](references/upgrades.md) instead of starting website work. For design, implementation or brand onboarding, read [workflow.md](references/workflow.md). It defines discovery, the two design approval gates, implementation and verification; stop at the current gate rather than completing later phases early.

- Project requirements and safety constraints apply throughout. Within those constraints, explicit brand rules and approved project choices take precedence over Taste's aesthetic defaults. Surface actual conflicts; absence of a rule is creative freedom, not a prohibition.
- For a named or missing brand, read [brands.md](references/brands.md). Use one selected local brand pack; keep project-specific design choices out of its sourced rules.
- For a new or materially revised direction, read [stitch.md](references/stitch.md). Stitch is the default, not a requirement for small changes to an approved design.
- For narrative or significant interaction motion, read [motion.md](references/motion.md). Mobile retains equal importance; geometry can change.
- When authoring or revising visible prose, read [the writing skill](../anti-ai-slop-writing/SKILL.md) and its applicable references. Brand voice and market spelling take precedence over its default register. Preserve supplied facts, quotations and protected source text.
- Before production implementation and final delivery, read [verification.md](references/verification.md). Quiet Impeccable hooks provide evidence, not creative authority.

Keep these files portable: use the current agent's file, shell, browser and MCP tools. No Claude-only slash command, hidden subagent requirement or automatic website deployment is part of this workflow.
...
1. **Page kind** - landing, portfolio, redesign (preserve vs overhaul), editorial, ecommerce, dashboard or other web application. Dense functional interfaces need clear tables, forms and repeatable patterns, not mandatory cinematic composition.
...
Ask only unresolved questions whose answers could change the direction. Group independent decisions when that saves an extra round; do not ask the user for facts available in the project. Example: *"Should this feel closer to Linear-clean or Awwwards-experimental?"*
...
**Starting reference:** `8 / 6 / 4` for expressive marketing work, not a universal preset. Infer all three values from the task and show them briefly with the Design Read. The user can override them conversationally; no extra approval gate is needed.
...
| Dashboard / functional web application | 3 | 2 | 7 |
...
### 3.A Stack
* **Framework:** Preserve the project's framework. React or Next.js are options for a new project, not migration requirements. Use Server Components only where the framework supports them.
  * **RSC SAFETY:** Global state works ONLY in Client Components. In Next.js, wrap providers in a `"use client"` component.
  * **INTERACTIVITY ISOLATION:** Any component using Motion, scroll listeners, or pointer physics MUST be an isolated leaf with `'use client'` at the top. Server Components render static layouts only.
* **Styling:** **Tailwind v4** (default). Tailwind v3 only if the existing project demands it.
  * For v4: do NOT use `tailwindcss` plugin in `postcss.config.js`. Use `@tailwindcss/postcss` or the Vite plugin.
* **Animation:** Native CSS, sticky positioning and scroll snap first; retain a suitable installed library. When React animation needs a library, Motion (`motion/react`) is an option; GSAP suits complex scroll choreography. Install only what the approved interaction requires.
* **Fonts:** Preserve the project's approved loading approach. Next.js can use `next/font`; self-hosted fonts should use `@font-face` with `font-display: swap`. Respect font licences and avoid adding a new external dependency without a reason.
...
### 3.C Icons
* **Priority:** approved brand icons, then the project's existing icon family. Phosphor, HugeIcons, Radix, Tabler and Lucide are options when none exists.
* Reuse assets before adding a dependency. Custom SVG is appropriate for approved brand artwork or a genuinely missing simple glyph.
* Keep the icon language consistent; do not add a second family merely for novelty.
...
* **SERIF DISCIPLINE (VERY DISCOURAGED AS DEFAULT):**
  ...
  * **If a serif fits**, consider the following examples, subject to brand rules and font licensing. Do not rotate away from a brand font for novelty: PP Editorial New, GT Sectra Display, Cardinal Grotesque, Reckless Neue, Tiempos Headline, Recoleta, Cormorant Garamond, Playfair Display, EB Garamond, IvyPresto, Migra, Editorial Old, Saol Display, Söhne Breit Kursiv, Domaine Display, Canela, Schnyder, Tobias, NB Architekt, ITC Galliard.
...
* Prefer a restrained accent palette when no brand palette exists. Multiple brand colours and distinct semantic success/warning/error colours are valid.
* **THE LILA RULE:** The "AI Purple / Blue glow" aesthetic is discouraged as a default. No automatic purple button glows, no random neon gradients. Use neutral bases (Zinc / Slate / Stone) with high-contrast singular accents (Emerald, Electric Blue, Deep Rose, Burnt Orange, etc.).
* **Override:** if the brand or brief explicitly asks for purple / violet / lila, embrace it. But execute with intent: consistent palette, harmonised neutrals, restrained gradients. Not generic AI gradient slop.
* **One palette per project.** Do not fluctuate between warm and cool grays within the same project.
* **Colour consistency:** use semantic colour roles consistently. Brand accents, interaction colours and status colours may differ for a documented reason; approved multiple palettes or themes are not detector defects.
...
* **PREMIUM-CONSUMER PALETTE BAN (mandatory, second-most-recurring AI-tell):**
  ...
  * **Palette choice:** base it on this brand and brief, not what happened in an unrelated previous project. Reusing an approved brand palette is correct.
...
* **BUTTON CONTRAST CHECK (mandatory, a11y):** Before shipping any button, verify the button text is readable against the button background. White button + white text, `bg-white` CTA with `text-white` label, transparent button against the page background with no border → all banned. Audit every CTA: contrast ratio WCAG AA min (4.5:1 normally; 3:1 at 24px regular or approximately 18.7px bold). Same rule applies to ghost buttons over photographic backgrounds (use a backdrop, scrim, or stroke).
...
### 4.7 Layout Discipline (Marketing composition defaults)

The following numerical caps are prompts to question clutter, not universal acceptance tests. Apply only where they fit the approved composition. Required content, useful repeated application patterns, responsive reflow and an approved scroll narrative take precedence. Never cut protected text or remove functionality to satisfy a cap.

* **Hero first impression:** make the purpose and next action easy to find. A compact marketing hero often fits the first viewport; an approved scroll narrative may span several. Preserve required copy and support zoom rather than forcing a line or word cap.
...
* **Layout rhythm:** avoid unthinking repetition on expressive pages. Repeat the same layout when items need direct comparison or users benefit from predictable scanning; do not manufacture variety in tables, forms or itineraries.
...
1. **Approved assets first.** Use supplied brand artwork, real product imagery and source-system assets. Image generation is optional for suitable creative material, never a replacement for factual depictions, real logos or required source images.
2. **Suitable licensed or generated assets second.** Check subject, rights and provenance. Random image seeds do not select the correct subject. Label prototype placeholders visibly; use the project's approved missing-image treatment in production.
...
**Images serve the surface.** Image-led experiences need strong imagery. A text-led design or functional tool may need none; do not add decorative images merely to satisfy a quota.
...
* Ensure logos render correctly against every supported background and theme; do not invent an unrequested light or dark mode.
...
**Hero needs a deliberate composition.** Strong typography can carry it without an image; an arbitrary gradient blob is not a substitute for design.

### 4.9 Content Density

Marketing summaries benefit from brevity. Functional pages and detailed experiences must retain all required content. The following marketing defaults do not justify hiding data needed for comparison or changing source text.
...
* **COPY SELF-AUDIT (mandatory before ship):** Before declaring any task done, re-read every visible string on the page (headlines, subheads, eyebrows, button labels, body copy, captions, alt text, footer text, error messages). Flag any string that is:
  ...
  Revise authored copy using the writing skill. Flag problems in supplied or protected text without changing it unless editing is authorised.
...
* Prefer brief quotations where suitable, but preserve exact quoted words and attribution. Use an authorised excerpt or an expandable full quotation rather than silently cutting the source.
* For very small font sizes (e.g. footer-style testimonials), the line cap can stretch slightly. Spirit: "fits in a glance."
* Preserve punctuation in quotations. For authored text, follow brand voice and the writing skill rather than a blanket punctuation ban.
* Preserve the supplied attribution. Include role or company only when known; do not invent missing details to fill a layout.
...
Keep theme changes deliberate and coherent. A single theme is a useful default; approved contrasting sections and multi-theme designs are valid.

* Avoid accidental inversions that make a section look as though it belongs to another site. Contrasting sections or theme changes are appropriate when they serve the approved composition; there is no arbitrary switch quota.
* Select light, dark, auto or multiple themes from the brief and approved direction. Section-level tints and intentional colour blocks are valid when their role is clear.
* When using a design system with built-in theming, apply themes through its intended root or scoped mechanism rather than scattering unrelated overrides.
...
* **All animation must respect `prefers-reduced-motion`, regardless of dial value.** Prefer static or immediate transitions; do not retain unnecessary waiting time.
...
### 6.C Theme selection
* Propose light, dark or both according to the brief and approved direction. Missing dark-mode guidance does not forbid creating a brand-consistent dark treatment.
* Add dual-mode behaviour only when selected; a toggle is not compulsory.
* Preserve brand recognition and WCAG AA contrast in every supported mode. Use the project's existing theming mechanism.

### 6.D Core Web Vitals Targets
* **LCP** < 2.5s. Load the main image through the framework's optimised image path or preload it where measurement justifies that choice.
...
Avoid arbitrary `z-50` or `z-10`. Use z-index for intentional layer contexts such as navigation, dialogs and overlays; reuse the project's scale or document one when several layers need coordination.
...
* **Responsive expression:** adapt geometry and density to the available space. Single-column is one option, not a requirement to remove the approved visual concept or motion from mobile.
...
* **8-10 (Cockpit):** Tight, readable spacing and clear grouping. Use cards, dividers and tabular or monospace numerals only where they improve scanning and comparison.

---

## 8. DARK MODE PROTOCOL

Apply this section only when the approved direction includes dark mode. A light-only, dark-only or dual-mode design can all be correct.
...
* Choose neutrals intentionally. Pure black and white are valid brand colours; off-black and off-white are alternatives, not mandates.

### 8.C Default Mode
Respect `prefers-color-scheme` unless the brand insists. Add a manual toggle if either mode would lose key brand expression.

### 8.D Test Every Supported Mode Before Finishing
Open every implemented mode during development; do not invent another mode just to satisfy a checklist.
...
* **Avoid automatic palette defaults.** Use the approved brand/project colours, including pure black when appropriate.
...
* **Facts retain their exact values**, including round numbers. Synthetic data must be labelled as such; do not invent realistic-looking metrics or contact details and present them as real.
...
* **Use approved brand icons or the existing icon family.** Custom SVG and Lucide are valid where appropriate; see Section 3.C.
...
* **Avoid punctuation as decoration.** Preserve source punctuation; apply the brand voice and writing guidance to authored copy (Section 9.G).
...
### 9.G Writing authority

Use the selected brand's voice and market conventions, then the bundled anti-ai-slop-writing skill for authored prose. It favours clear, supported statements rather than mechanical word or punctuation bans. Supplied facts, quotations and protected source text remain unchanged; report concerns instead of silently editing them.
...
* **One owner per animated property.** GSAP, Three.js and Motion may coexist when they control distinct elements/properties. A reduced-motion hook does not itself animate anything. Do not add Motion solely for preference detection when the project already has an equivalent.
...
Apply this workflow to all websites and web applications, including dashboards, tables, multi-step forms and embedded editors. Adapt the dials, use semantic controls and reuse mature components where necessary. Do not replace useful functional patterns with marketing-page decoration.

Native applications, backend architecture and collaboration algorithms need their own implementation expertise; this skill governs their web presentation only.

---

## 14. FINAL PRE-FLIGHT CHECK

Run this matrix after implementation alongside [verification.md](references/verification.md). Report pass, fail, not applicable or not checked with evidence. Aesthetic defaults below apply only where the approved direction adopts them; they cannot override brand rules, source content, useful application patterns or approved exceptions. Required checks that failed or were not run remain visible blockers.
...
- [ ] **Writing:** authored copy follows brand voice and the writing skill; supplied facts, quotations and protected text are unchanged.
- [ ] **Theme coherence:** every supported mode and contrasting section follows the approved composition.
- [ ] **Colour roles:** brand, interaction and status colours are applied consistently.
...
- [ ] **Typography:** fonts suit the brand/direction, licences permit use and loaded fonts match the rendering.
- [ ] **Palette:** selected for this brand and surface, not a generic luxury preset or a novelty rotation.
- [ ] **Italic descender clearance**: every italic word with `y g j p q` has `leading-[1.1]` min + `pb-1` reserve?
- [ ] **Hero:** purpose and next action are clear; approved scroll narratives, required content and zoom remain usable.
...
- [ ] **Layout rhythm:** repetition supports scanning or comparison; variation serves the approved composition rather than a quota.
...
- [ ] **Assets:** approved supplied assets take priority; subjects and rights are checked; placeholders are visible and no image quota is imposed.
...
- [ ] **Quotations:** exact source wording and attribution preserved; excerpts are authorised.
...
- [ ] **Reduced motion:** all animations respect the preference, without unnecessary delays or lost access to content.
- [ ] **Supported themes:** each selected mode tested; no unrequested dual-mode requirement.
- [ ] **Mobile:** responsive geometry retains the approved concept, content and functionality, including touch interaction.
- [ ] **Viewport stability**: `min-h-[100dvh]`, never `h-screen`?
- [ ] **`useEffect` animations** have strict cleanup functions?
- [ ] **Empty / loading / error** states provided?
- [ ] **Cards omitted** in favor of spacing where possible?
- [ ] **Icons:** approved brand assets or a consistent existing family, without unnecessary dependencies.
- [ ] **Motion ownership:** animated properties have one owner; framework-specific client boundaries and cleanup are correct where applicable.
- [ ] **No AI Tells** from Section 9 (Inter as default, AI-purple, three-equal cards, Jane Doe, Acme, "Quietly in use at")?
- [ ] **Performance:** measurements and limitations recorded; lab checks do not establish field INP or production readiness.
- [ ] **One design system** per project (no Material + shadcn mixed)?

Deliver only what the evidence supports. Fix required failures, explain legitimate exceptions, and disclose untested conditions rather than converting every box to a pass.
