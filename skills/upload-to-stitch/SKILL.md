...
The key is `STITCH_API_KEY` in `~/.claude/skills/ui-dev/.env`. Load it into the shell rather than
reading its value, so the key never appears in the conversation. If the file or the value is
missing, ask the user to add it there; do not proceed without it, and do not look for the key
anywhere else.
...
```bash
set -a; . ~/.claude/skills/ui-dev/.env; set +a
