...
Every candidate passes four questions, in order, and the answers go in the report:

1. **Frequency.** Seen 100+ times a day (keyboard shortcuts, command palette, core navigation): reject, no animation, ever. Tens of times a day (hover states, list navigation, frequent toggles): reject, or near-imperceptible motion only. Occasionally (modals, toasts, sheets): allowed within budget. Rarely (first run, success, empty states): the delight budget lives here.
2. **Purpose.** Name it in one word: feedback, continuity, orientation, hierarchy, or delight (delight only at the rare tier). "It looks cool" is not a purpose; reject.
3. **Speed.** It stays inside the duration budget for its tier. If the moment only works as a slow, showy animation, reject.
4. **Function.** Decoration on functional, information-dense UI hinders. Data the user is reading or acting on does not move for style; reject.

Report at most a handful that survive, grounded in actual UX seams you observed, not a wishlist. Then list two to five candidates you rejected, each with the question that killed it, because what was deliberately left still is what separates an audit from a wishlist.
