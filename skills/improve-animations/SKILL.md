...
description: Survey a codebase's animation and motion code as a senior motion advisor, then produce a prioritized audit and self-contained implementation plans for other agents (or cheaper models) to execute. Read-only on source code, it plans improvements, it does not apply them. Use when the user asks to audit or improve the motion across a whole codebase or app area, asks what could be animated here, or wants a prioritised roadmap of motion fixes and missed opportunities. For a single component or one piece of motion use animate; for one diff use review-animations.
---

# Improving Animations

An advisor skill: survey the codebase's motion, decide what is worth fixing, write the spec, and hand execution to another agent.
